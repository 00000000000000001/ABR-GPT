...
def create_test_doc():
    ...
    document.add_paragraph("""Text oberhalb des GPT-Blocks""")

    document.add_paragraph("""{{Anamnese, Untersuchung, Indikationsstellung:
    ...
    Ausführliche schriftliche und mündliche Aufklärung des Patienten über eventuelle Komplikationen der MRT-Untersuchung bei 3 Tesla Feldstärke.""")

    document.add_paragraph("""Technik:
    ...
    PD-gewichtete TSE-Sequenzen mit SPAIR-Fettsuppression in sagittaler, axialer, coronarer Orientierung. Sagittale T1-gewichtete TSE-Sequenzen, isometrische PD-gewichtete 3D-TSE-Sequenzen mit Erzeugung mehrerer multiplanarer Reformationen.""")

    document.add_paragraph("""Befund:
    ...
    Unauffällige Haut sowie Unterhautfettgewebe.""")

    document.add_paragraph("""Beurteilung:
    ...
    deutliche arthrotische Veränderungen auch im proximalen Tibiofibulargelenk.""")

    document.add_paragraph("""Procedere:
    Im Anschluss an die Untersuchung erfolgte die ausführliche Befundbesprechung und Bilddemonstration an Herrn Mustermann.}}""")

    document.add_paragraph("""Text unterhalb des GPT-Blocks""")

    document.save("./Arztbrief_Mustermann.docx")
